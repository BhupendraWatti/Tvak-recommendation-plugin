...
class Tvak_Hamper {
    ...
    private static function get_tvak_shade_options($product): array {
        if (!$product || !class_exists('Tvak_Product_Shade') || !Tvak_Product_Shade::get_product_has_shades((int) $product->get_id())) {
            return [];
        }

        $options = [];
        $by_variation_id = [];
        $default_hex = class_exists('Tvak_Shade_Sync') ? strtolower((string) Tvak_Shade_Sync::get_default_hex()) : strtolower((string) get_option('tvak_default_shade_hex', ''));
        foreach (Tvak_Product_Shade::get_shades_by_product((int) $product->get_id(), true) as $shade) {
