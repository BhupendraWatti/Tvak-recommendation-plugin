...
class Tvak_Custom_Hamper_WooCommerce {
    ...
    public static function enqueue_assets(): void {
        ...
        if (!$product_id || !Tvak_Custom_Hamper::is_hamper_product((int) $product_id)) {
            return;
        }

        wp_enqueue_style('tvak-hamper-css', TCHB_PLUGIN_URL . 'assets/css/tvak-hamper.css', [], TCHB_VERSION);
        wp_enqueue_script('tvak-hamper-js', TCHB_PLUGIN_URL . 'assets/js/tvak-hamper.js', ['jquery'], TCHB_VERSION, true);
    ...
    public static function render_builder(): void {
        ...
        if (!$product || !Tvak_Custom_Hamper::is_hamper_product((int) $product->get_id())) {
            return;
        }
        ...
        <div id="tvak-hamper-builder" class="tvak-hamper-builder" data-product-id="<?php echo esc_attr($product->get_id()); ?>">
            <div class="tvak-hamper-loading">
                ...
                <p><?php esc_html_e('Loading your custom hamper...', 'tvak-custom-hamper-builder'); ?></p>
            </div>
        </div>
        <?php
    }
    ...
    public static function block_direct_hamper_shell_add($passed, $product_id, $quantity, $variation_id = 0, $variations = [], $cart_item_data = []): bool {
        ...
        if ($product_id && Tvak_Custom_Hamper::is_hamper_product((int) $product_id)) {
            wc_add_notice(__('Please build your hamper before adding it to cart.', 'tvak-custom-hamper-builder'), 'error');
            return false;
        }
    ...
    public static function handle_get_hamper(WP_REST_Request $request): WP_REST_Response {
        ...
        $payload = Tvak_Custom_Hamper::build_payload_for_product($product_id);

        if (!$payload) {
            return new WP_REST_Response([
                'success' => false,
                'message' => __('This product is not an active TVAK hamper.', 'tvak-custom-hamper-builder'),
            ], 404);
        }
    ...
    public static function handle_add_hamper(WP_REST_Request $request): WP_REST_Response {
        if (is_user_logged_in() && !wp_verify_nonce($request->get_header('X-WP-Nonce'), 'wp_rest')) {
            return new WP_REST_Response([
                'success' => false,
                'message' => __('Security check failed.', 'tvak-custom-hamper-builder'),
            ], 403);
        }
    ...
    public static function process_add_hamper_to_cart(array $payload): array {
        if (!class_exists('WooCommerce') || !WC()->cart) {
            return ['success' => false, 'message' => __('WooCommerce cart is not available.', 'tvak-custom-hamper-builder')];
        }
        ...
        $hamper            = Tvak_Custom_Hamper::build_payload_for_product($hamper_product_id);

        if (!$hamper) {
            return ['success' => false, 'message' => __('Invalid or inactive hamper product.', 'tvak-custom-hamper-builder')];
        }
        ...
        foreach ($submitted_items as $raw_item) {
            ...
            if (!$product_id || !isset($allowed_items[$product_id])) {
                return ['success' => false, 'message' => __('The hamper contains an item that is not allowed.', 'tvak-custom-hamper-builder')];
            }
        ...
        foreach ($allowed_items as $product_id => $definition) {
            if (!empty($definition['is_required']) && !isset($selected[$product_id])) {
                return ['success' => false, 'message' => __('This hamper is missing a required product.', 'tvak-custom-hamper-builder')];
            }
        }
        ...
        if ($selected_count < (int) $hamper['min_items'] || $selected_count > (int) $hamper['max_items']) {
            return [
                'success' => false,
                'message' => sprintf(
                    __('Please select between %1$d and %2$d products for this hamper.', 'tvak-custom-hamper-builder'),
        ...
        foreach ($selected as $product_id => $item) {
            ...
            if (!$product || !$product->is_purchasable() || !$product->is_in_stock()) {
                return ['success' => false, 'message' => __('One selected hamper product is unavailable.', 'tvak-custom-hamper-builder')];
            }
            ...
            if ($product->is_type('variable')) {
                if (!$variation_id) {
                    return ['success' => false, 'message' => sprintf(__('Please choose options for %s.', 'tvak-custom-hamper-builder'), $product->get_name())];
                }

                $variation = wc_get_product($variation_id);
                if (!$variation || (int) $variation->get_parent_id() !== $product_id || !$variation->is_purchasable() || !$variation->is_in_stock()) {
                    return ['success' => false, 'message' => sprintf(__('The selected variation for %s is unavailable.', 'tvak-custom-hamper-builder'), $product->get_name())];
                }
            } else {
                $variation = null;
                $variation_id = 0;
        ...
        if (!$included_items) {
            return ['success' => false, 'message' => __('No hamper products were added to cart.', 'tvak-custom-hamper-builder')];
        }
        ...
        if (!$cart_item_key) {
            return ['success' => false, 'message' => __('The hamper could not be added to cart.', 'tvak-custom-hamper-builder')];
        }

        return [
            'success'      => true,
            'message'      => sprintf(__('Hamper added to your bag with %d selected products.', 'tvak-custom-hamper-builder'), count($included_items)),
    ...
    public static function display_cart_item_meta(array $item_data, array $cart_item): array {
        if (!empty($cart_item['_tvak_is_custom_hamper_item'])) {
            ...
            $item_data[] = ['key' => __('Hamper', 'tvak-custom-hamper-builder'), 'value' => esc_html($cart_item['_tvak_hamper_title'] ?? '')];
            $item_data[] = ['key' => __('Included Items', 'tvak-custom-hamper-builder'), 'value' => esc_html(sprintf(_n('%d product selected', '%d products selected', $item_count, 'tvak-custom-hamper-builder'), $item_count))];
            if ($included_items) {
                $item_data[] = ['key' => __('Products Inside', 'tvak-custom-hamper-builder'), 'value' => esc_html(self::format_included_items_summary($included_items))];
            }
            $item_data[] = ['key' => __('Hamper Ref', 'tvak-custom-hamper-builder'), 'value' => esc_html($cart_item['_tvak_hamper_cart_id'] ?? '')];
        }
        return $item_data;
    }
    ...
    public static function attach_line_item_meta($item, $cart_item_key, $values, $order): void {
        if (!empty($values['_tvak_is_custom_hamper_item'])) {
            ...
            $item->add_meta_data(__('Products Inside', 'tvak-custom-hamper-builder'), self::format_included_items_summary($values['_tvak_hamper_items'] ?? []));
        }
    }
    ...
    public static function render_admin_order_panel($order): void {
        ...
        <div class="order_data_column" style="width: 100%; margin-top: 20px; padding: 15px; background: #fff7ed; border: 1px solid #f59e0b; border-radius: 4px;">
            <h3><?php esc_html_e('TVAK Custom Hamper Order', 'tvak-custom-hamper-builder'); ?></h3>
            <p><strong><?php esc_html_e('Hamper Reference:', 'tvak-custom-hamper-builder'); ?></strong> <code><?php echo esc_html($order->get_meta('_tvak_hamper_cart_id')); ?></code></p>
            <p><em><?php esc_html_e('Pack the products listed inside the hamper line item together in the selected hamper packaging.', 'tvak-custom-hamper-builder'); ?></em></p>
        </div>
        <?php
    }
